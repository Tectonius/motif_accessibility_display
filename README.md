...
Two analyses, each run against both tracks:
...
nucleosome_detection}` and their DNase-I-only counterparts
(`dnase_motif_accessibility_profiles`, `dnase_nucleosome_detection`). See
`project_outlines/chrombpnet_hic_accessibility.md` in the parent research
repo for the full methodology behind the DNase-I-only subset.
